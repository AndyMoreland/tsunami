...
(defun tsunami-refactor-implement-interface ()
  ...
  (interactive)
  (tsunami--command:implement-interface (buffer-file-name (current-buffer)) (1- (point))
                                        (lambda (response)
                                          (if (tide-response-success-p response )
                                              (when (not (eq :json-null (plist-get response :body)))
                                                (tsunami--apply-code-edit (plist-get response :body)))
                                            (message "Failed to implement interface.")))))
