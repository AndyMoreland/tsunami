...
function emitCallSignature(
    ...
    const nameAndTypeTuples = signature
        .getParameters()
        .map(param => [
            param.getName(),
            checker.typeToString(
                checker.getTypeAtLocation(param.getDeclarations()![0])
            )
        ]);

    const returnType = checker.typeToString(signature.getReturnType());

    return `(${nameAndTypeTuples
...
export class ImplementInterfaceCommandDefinition
    implements CommandDefinition<ImplementInterfaceCommand, CodeEdit | null> {
    ...
    public async processor(
        ...
        const classNode = containingNodes.filter(
        ...
        const methodDescriptors = props
            .filter(prop => !childNames.has(prop.getName()))
            .map(prop => {
                let result = prop.getName();
                const propType = checker.getTypeAtLocation(
                    prop.getDeclarations()![0]
                );

                if (isCallableType(propType)) {
                    result += emitCallSignature(
                        checker,
