...
export function isExportedNode(node: ts.Node): boolean {
    return (
        ...
        node.modifiers.filter(
            modifierNode => modifierNode.kind === ts.SyntaxKind.ExportKeyword
        ).length > 0
    );
}
...
export function isExpressionWithTypeArgumentsInClassExtendsClause(
    node: ts.Node
): boolean {
    return (
        node.kind === ts.SyntaxKind.ExpressionWithTypeArguments &&
        (<ts.HeritageClause>node.parent).token ===
        ...
        case ts.SyntaxKind.ThisKeyword:
            ...
            switch (parent!.kind) {
                ...
                case ts.SyntaxKind.BindingElement:
                    return (
                        (<ts.VariableDeclaration>parent).initializer === node
                    );
                ...
                case ts.SyntaxKind.ComputedPropertyName:
                    return (
                        node === (<ts.ComputedPropertyName>parent).expression
                    );
                ...
                case ts.SyntaxKind.ExpressionWithTypeArguments:
                    return (
                        (<ts.ExpressionWithTypeArguments>parent).expression ===
                            node &&
...
export function convertPositionToLocation(
...
export function getNodesContainingPoint(
    sourceFile: ts.SourceFile,
    point: number
) {
